<div class="row">
    <div class="col-sm-6">
        ...
        <div class="form-group {{ $errors->has('e_bom_assy_id') ? ' has-error' : '' }}">
            <label>Engineering BOM Assy</label>
            {!! Form::select('e_bom_assy_id', [
                '' => ''
                ] + App\EBOMAssy::pluck('part_name','id')->all(), null, ['class'=>'form-control']); !!}
            @if($errors->has('e_bom_assy_id'))
                <span id="form_control_1-error" class="help-block help-block-error">{{ $errors->first('e_bom_assy_id') }}</span>            
            @endif
        </div>
...
<div class="row">
	<div class="col-md-12">
		<hr>
        <center>
            <a href="{{ url('engineering/e-bom/single-parts') }}" class="btn btn-default">
